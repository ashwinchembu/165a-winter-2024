	...
	if (page_range_update[i] >= MAX_PAGE_RANGE_UPDATES){
		...
		for (int i = deep_copy->pages.size() - 1; i >= 0; i--) {
			...
			for (int to_load_tail_page_col = 0; to_load_tail_page_col < num_columns + NUM_METADATA_COLUMNS; to_load_tail_page_col++){
				...
				page_pointer->data = new int[PAGE_SIZE];
				...
				std::cout << "what is in frame, pagerange RID: " << add_to_queue->first_rid_page_range << " RID: " << add_to_queue->first_rid_page << std::endl;

				//delete[] page_pointer->data;
				// delete page_pointer;
				
				insert_to_queue.push_back(add_to_queue);
			}
		}
		//std::cout << std::endl;
