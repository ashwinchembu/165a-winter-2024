...
Frame* BufferPool::search(const RID& rid, const int& column){
  ...
  while(current_frame != range_end->next){
    std::cout << "search : " <<current_frame << " current frame->next: " << current_frame << " first_rid_page: " << current_frame->first_rid_page << " column: " << current_frame->column << " pin: " << current_frame->pin << std::endl;
